...
import com.example.appreadbook.databinding.ActivityAddCategoryBinding;
...
public class AddCategoryActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding.backadd.setOnClickListener(v->onBackPressed());
    }
    public void updatecate(){
        if(checkfild()){
            Call<String> productCall= ApiClient.getUserServicd().updatecate(categorydata.getTitle(),categorydata.getDescription(),binding.title.getText().toString(),encodedImage,binding.description.getText().toString());
            productCall.enqueue(new Callback<String>() {
                @Override
                public void onResponse(Call<String> call, Response<String> response) {
                    Toast.makeText(getApplication(), "Sửa thành công",Toast.LENGTH_LONG).show();
                    String message=response.body();
                    Intent intent=new Intent(AddCategoryActivity.this,SliderCategory.class);
                ...
                public void onFailure(Call<String> call, Throwable t) {
                    Toast.makeText(getApplication(), "Sửa không thành công",Toast.LENGTH_LONG).show();
                }
            });
        }
    }
    private void checkupdate(){
        ...
        if((Categorydata)getIntent().getSerializableExtra("Loai")!=null){
              Toast.makeText(getApplication(), "update",Toast.LENGTH_LONG).show();
            update=true;
            categorydata=(Categorydata) getIntent().getSerializableExtra("Loai");

            binding.title.setText(categorydata.getTitle());
            binding.description.setText(categorydata.getDescription());
            binding.buttonadd.setText("Update");
            encodedImage=categorydata.getIcon();
            binding.imgtruyen.setImageBitmap(getImage(encodedImage));
        }
    }
    public void add(){
        if(checkfild()){
            Call<String> productCall= ApiClient.getUserServicd().addcate(binding.title.getText().toString(),encodedImage,binding.description.getText().toString());
            productCall.enqueue(new Callback<String>() {
                @Override
                public void onResponse(Call<String> call, Response<String> response) {
    ...
    public boolean checkfild(){
        if(binding.title.getText().toString().trim().length()==0){
            Toast.makeText(AddCategoryActivity.this,"Bạn phải nhập tiêu đề ",Toast.LENGTH_LONG).show();
            return false;
        }
        if(binding.description.getText().toString().trim().length()==0){
            Toast.makeText(AddCategoryActivity.this,"Bạn phải nhập Mô tả ",Toast.LENGTH_LONG).show();
            return false;
        }
        if(encodedImage.length()==0&&update==false){
            Toast.makeText(AddCategoryActivity.this,"Bạn phải nhập ",Toast.LENGTH_LONG).show();
            return false;
        }
        return true;
    }
